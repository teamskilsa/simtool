// LTE antenna configuration — count + TX/RX gain. Same per-antenna toggle as
// the NR AntennaSection. Amarisoft tx_gain / rx_gain may be a scalar applied
// to every antenna path or an array with one value per path.
...
export function AntennaSectionLte({ form, onChange }: Props) {
  ...
  const handleAntennaCountChange = (key: 'nAntennaDl' | 'nAntennaUl', value: number) => {
    ...
    if (key === 'nAntennaDl' && txIsArray) {
      ...
      const fill = arr[arr.length - 1] ?? defaultGains('lte', value).txGain;
